#!/usr/bin/env python3
"""
GUI for A* Algorithm Visualization
...
class AStarGUI:
    def __init__(self, root):
        ...
        self.custom_filename = tk.StringVar(value="astar_animation")
    ...
    def create_widgets(self):
        ...
        # Title
        title_label = ttk.Label(left_panel, text="A* Algorithm Settings", 
                               font=('Arial', 14, 'bold'))
        ...
        ttk.Label(left_panel, text="Heuristic: Manhattan Distance", 
                 font=('Arial', 9, 'italic'), foreground='blue').grid(row=row_idx, column=0, columnspan=2, pady=5)
        row_idx += 1
    ...
    def create_animation(self, rows, cols, start, goal, obstacles, result):
        ...
        viz_window.title("A* Algorithm - Live Simulation")
        ...
        def animate_step():
            ...
            if phase == 0:  # Exploration phase
                if current_frame[0] < len(explored_nodes):
                    ...
                    im.set_array(grid)
                    title_text.set_text(
                        f"A* Exploring Nodes - Step {current_frame[0] + 1}/{len(explored_nodes)}"
                    )
                    status_var.set(
                ...
                if current_frame[0] < len(path_nodes):
                    ...
                    im.set_array(grid)
                    title_text.set_text(
                        f"Drawing Optimal Path - Step {current_frame[0] + 1}/{len(path_nodes)}"
                    )
                    status_var.set(
                    ...
                    # Animation complete
                    is_running[0] = False
                    title_text.set_text("A* Simulation Complete!")
                    status_var.set(
